...
(function (global, factory) {
    ...
    return _createClass(TagCloud, [{
      ...
      value: function _calculateDimensions() {
        ...
        if (self.config.width && self.config.height) {
          ...
          var containerWidth = self.$container.clientWidth || parseInt(containerStyle.width, 10) || self.config.width || 300; // fallback default
          var containerHeight = self.$container.clientHeight || parseInt(containerStyle.height, 10) || self.config.height || 300; // fallback default
      ...
      function _createElment() {
        ...
        if (self.config.useContainerInlineStyles) {
          ...
          $el.style.width = "".concat(self.containerWidth, "px");
          $el.style.height = "".concat(self.containerHeight, "px");
        }
      ...
      value: function _computePosition(index) {
        ...
        var scaleX = self.containerWidth / (2 * self.radius);
        var scaleY = self.containerHeight / (2 * self.radius);
        var scaleZ = 1; // keep Z axis as sphere for depth effect
        return {
      ...
      value: function _setupResizeObserver() {
        var self = this;
        ...
        if (typeof window.ResizeObserver !== 'undefined') {
            ...
            try {
              for (_iterator.s(); !(_step = _iterator.n()).done;) {
                ...
                if (entry.target === self.$container) {
                  var newWidth = entry.contentRect.width || self.$container.clientWidth;
                  var newHeight = entry.contentRect.height || self.$container.clientHeight;

                  // Only resize if dimensions actually changed
                  var widthChanged = newWidth !== self.containerWidth;
                  var heightChanged = newHeight !== self.containerHeight;
          ...
          TagCloud._on(window, 'resize', function () {
      ...
      key: "_handleResize",
      value: function _handleResize(newWidth, newHeight) {
        var self = this;

        // Add minimum change threshold to prevent tiny fluctuations
        var minChangeThreshold = 1; // 1px minimum change
        var widthDiff = Math.abs(newWidth - self.containerWidth);
        var heightDiff = Math.abs(newHeight - self.containerHeight);
      ...
      key: "_performResize",
      value: function _performResize(newWidth, newHeight) {
        var self = this;
        ...
        if (self.config.useContainerInlineStyles) {
          ...
          var parentStyle = window.getComputedStyle(self.$container);
          var parentHasFixedWidth = parentStyle.width !== 'auto' && !parentStyle.width.includes('%');
          var parentHasFixedHeight = parentStyle.height !== 'auto' && !parentStyle.height.includes('%');

          // Only set dimensions if parent has fixed size to prevent feedback loops
          if (parentHasFixedWidth || self.config.width) {
            self.$el.style.width = "".concat(self.containerWidth, "px");
          }
          if (parentHasFixedHeight || self.config.height) {
            self.$el.style.height = "".concat(self.containerHeight, "px");
          }
        }

        // Recalculate all item positions
        self.items.forEach(function (item, index) {
          var newPosition = self._computePosition(index);
  ...
  TagCloud._defaultConfig = {
    radius: 100,
    // rolling radius, unit `px` (deprecated, will be calculated from container size)
    width: null,
    // container width, if null will use parent container width
    height: null,
    // container height, if null will use parent container height
    maxSpeed: 'normal',
    ...
    hls: false,
    // whether to use HSL color based on weight
